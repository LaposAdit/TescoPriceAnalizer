...
import { StarostlivostODomacnostTransformedProductDto, PromotionDto, StarostlivostODomacnostResponseDto} from 'src/dto/starostlivost-o-domacnostDTO';


@Injectable()
export class StarostlivostODomacnostService {
    ...
    private async fetchProductsFromApi(): Promise<StarostlivostODomacnostTransformedProductDto[]> {
        const url = 'https://potravinydomov.itesco.sk/groceries/sk-SK/resources';
        const headers = { ...defaultHeaders }

        let allProducts: StarostlivostODomacnostTransformedProductDto[] = [];
        let count = 48;
        let page = 1;
        let morePages = true;
        ...
        while (morePages) {
            const body = {
                resources: [
                    {
                        ...
                        params: {
                            ...
                            superdepartment: 'starostlivost-o-domacnost'
                        },
                ...
                sharedParams: {
                    superdepartment: 'starostlivost-o-domacnost',
                    query: { count: count.toString(), page: page.toString() },
                    referer: `/groceries/sk-SK/shop/starostlivost-o-domacnost/all?count=${count}&page=${page - 1}`
                },
    ...
    private transformData(data: any): StarostlivostODomacnostTransformedProductDto[] {
    ...
    private async saveProductsToDb(products: StarostlivostODomacnostTransformedProductDto[]) {
        for (const product of products) {
            try {
                // Create a new product record with a unique lastUpdated value
                await this.prisma.starostlivostODomacnost.create({
                    data: {
                        ...
                        isForSale: product.isForSale,
                        aisleName: product.aisleName,
                        category: 'starostlivost-o-domacnost',
                        superDepartmentName: product.superDepartmentName,
    ...
    async getProducts(update: boolean, page: number, pageSize: number, sale?: boolean): Promise<StarostlivostODomacnostResponseDto> {
        if (update) {
            const productsFromApi = await this.fetchProductsFromApi();
            await this.saveProductsToDb(productsFromApi);
        }

        const whereClause: any = { category: 'starostlivost-o-domacnost' };
        ...
        const [productsFromDb, totalProducts] = await this.prisma.$transaction([
            this.prisma.starostlivostODomacnost.findMany({
                where: whereClause,
            ...
            this.prisma.starostlivostODomacnost.count({
                where: whereClause
            })
        ]);
    ...
    async getProductById(productId: string): Promise<StarostlivostODomacnostTransformedProductDto[]> {
        const productsFromDb = await this.prisma.starostlivostODomacnost.findMany({
            where: { productId },
            include: { promotions: true },
            orderBy: { lastUpdated: 'desc' }
        });
