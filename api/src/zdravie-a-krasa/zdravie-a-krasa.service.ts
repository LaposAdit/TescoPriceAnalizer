...
import { ZdravieAKrasaServiceResponseDto, ZdravieAKrasaServiceTransformedProductDto, PromotionDto } from 'src/dto/zdravie-a-krasaDTO';
import { defaultHeaders } from 'src/common/header';

@Injectable()
export class ZdravieAKrasaService {
    ...
    private async fetchProductsFromApi(): Promise<ZdravieAKrasaServiceTransformedProductDto[]> {
        const url = 'https://potravinydomov.itesco.sk/groceries/sk-SK/resources';
        const headers = { ...defaultHeaders }

        let allProducts: ZdravieAKrasaServiceTransformedProductDto[] = [];
        let count = 48;
        let page = 1;
        let morePages = true;
        ...
        while (morePages) {
            const body = {
                resources: [
                    {
                        ...
                        params: {
                            ...
                            superdepartment: 'zdravie-a-krasa'
                        },
                ...
                sharedParams: {
                    superdepartment: 'zdravie-a-krasa',
                    query: { count: count.toString(), page: page.toString() },
                    referer: `/groceries/sk-SK/shop/zdravie-a-krasa/all?count=${count}&page=${page - 1}`
                },
    ...
    private transformData(data: any): ZdravieAKrasaServiceTransformedProductDto[] {
    ...
    private async saveProductsToDb(products: ZdravieAKrasaServiceTransformedProductDto[]) {
        for (const product of products) {
            try {
                // Create a new product record with a unique lastUpdated value
                await this.prisma.zdravieAKrasa.create({
                    data: {
    ...
    async getProducts(update: boolean, page: number, pageSize: number, sale?: boolean): Promise<ZdravieAKrasaServiceResponseDto> {
        if (update) {
            const productsFromApi = await this.fetchProductsFromApi();
            await this.saveProductsToDb(productsFromApi);
        }

        const whereClause: any = { category: 'zdravie-a-krasa' };
        ...
        const [productsFromDb, totalProducts] = await this.prisma.$transaction([
            this.prisma.zdravieAKrasa.findMany({
                where: whereClause,
            ...
            this.prisma.zdravieAKrasa.count({
                where: whereClause
            })
        ]);
    ...
    async getProductById(productId: string): Promise<ZdravieAKrasaServiceTransformedProductDto[]> {
        const productsFromDb = await this.prisma.zdravieAKrasa.findMany({
            where: { productId },
            include: { promotions: true },
            orderBy: { lastUpdated: 'desc' }
        });
